...
class LeaderboardControllerTest {
    ...
    fun getLeaderboard_ServiceDidNotFindLeaderboardForGivenId_Returns404() {
        whenever(leaderboardService.findLeaderboardWithPlayers("1")).thenReturn(null)
    ...
    fun getLeaderboard_ServiceDidFindLeaderboardForGivenId_Returns200() {
        val leaderboard = LeaderboardWithPlayers(UUID.randomUUID(), "OvOTxT", "shroodSquad", emptyList())
        whenever(leaderboardService.findLeaderboardWithPlayers("OvOTxT")).thenReturn(leaderboard)

        val contentAsString: String = mockMvc.perform(MockMvcRequestBuilders.get("/api/leaderboard/{id}", "OvOTxT"))
                .andExpect(MockMvcResultMatchers.status().`is`(200))
                .andReturn().getResponse().getContentAsString()

        Ass.assertThat(ObjectMapperFactory.json(contentAsString, LeaderboardWithPlayers::class.java)).isEqualTo(leaderboard)
    }
